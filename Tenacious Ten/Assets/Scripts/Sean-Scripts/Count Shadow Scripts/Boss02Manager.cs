...
public class Boss02Manager : MonoBehaviour {
    ...
    void Start()
    {
        Application.targetFrameRate = 73;
        ...
        CastSeals = FindObjectOfType<Phase2CastSeal>();
	...
	void FixedUpdate ()
    {
        ...
        if(phaseTwoComplete && StartFight) //if (EHM.enemyHealth < PhaseThreeHP) //phase 3
        {
            ...
            if (transform.position.x <= 8.78 && !XmoveCorrect && StartPhase3 <= 0)
            {
                transform.position = new Vector3(transform.position.x + 0.1f, transform.position.y, transform.position.z);
            }
            if (transform.position.x >= 8.78 && transform.position.y <= -1.5 && !hasTeleported && !stepOnePhase3 && isIdle) //first part of entering phase, drift boss to platform level
            {
                if(!facingLeft)
                {
                    Flip();
            ...
            if (isInvisible && !hasTeleported)
            {
                ...
                transform.position = new Vector3(somePlatform.transform.position.x, somePlatform.transform.position.y + 3.2f, somePlatform.transform.position.z);
            ...
            if (hoverBack)
            {
                ...
                if (fadeTimer <= 0 && hoverBack)
                {
                    ...
                    {
                        fadeAway = 0.1f;
                        transform.position = new Vector3(8.78f, -1.4f, 0f);
                        GetComponent<SpriteRenderer>().color = new Color(1, 1, 1, 1);
                        hoverBack = false;
                        isInvisible = false;
                        hasTeleported = false;
